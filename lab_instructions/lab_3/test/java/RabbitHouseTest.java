...
public class RabbitHouseTest {
	...
	void testInput1() {
		...
		ByteArrayInputStream in = new ByteArrayInputStream("normal\nPEWEW\n".getBytes());
		System.setIn(in);
	...
	void testInput3() {
		...
		// Mengecek apakah hasil-hasil pemrosesan ada pada output
		// Cek jawaban
		if (!outContent.toString().contains("11001082397556421")) {
			fail("Gagal testcase 3 HINT : Apa yang terjadi jika input terlalu panjang? Berapa batas int?");
		}
	}

	@Test
	@DisplayName("Testcase Input Langsung: Contoh Soal 4")
	void testInput4() {
		// Setup masukan menggunakan ByteArrayInputStream
		ByteArrayInputStream in = new ByteArrayInputStream("normal\nJ\n".getBytes());
		System.setIn(in);
